...
function classifyResponse(status: number, body: string): AdapterEnvironmentCheck {
  ...
  if (status === 401) {
    return {
      ...
      detail,
      hint: "Verify ANTHROPIC_API_KEY and workspace/org routing.",
    };
  }
  if (status === 403) {
    return {
      ...
      level: "error",
      message: "Anthropic denied access to this request.",
      detail,
      hint: "Check model entitlements and organization-level permissions.",
    };
  }
  if (status === 429) {
    return {
      code: "anthropic_api_quota_exceeded",
      level: "warn",
      message: "Anthropic reported a rate-limit or quota issue.",
      detail,
      hint: "Check Anthropic billing, spend caps, and provider rate limits.",
    };
  }
  if (/model/i.test(body) && /not found|unknown|invalid|does not exist/i.test(body)) {
    return {
      ...
      level: "error",
      message: "The configured Anthropic model is invalid or unavailable.",
      detail,
      hint: "Pick a supported Anthropic model id.",
    };
  }
  return {
    code: "anthropic_api_probe_failed",
    level: status >= 500 ? "warn" : "error",
    message: "Anthropic hello probe failed.",
    detail,
  };
}
...
export async function testEnvironment(
  ...
  try {
    ...
    checks.push({
      code: "anthropic_api_cwd_valid",
      level: "info",
      message: `Working directory is valid: ${cwd}`,
    });
  } catch (err) {
    checks.push({
      code: "anthropic_api_cwd_invalid",
      level: "error",
  ...
  if (!apiKey) {
    checks.push({
      ...
      hint: "Bind ANTHROPIC_API_KEY through adapter env or the server environment.",
    });
  }
  ...
  if (!model) {
    checks.push({
      code: "anthropic_api_model_missing",
      level: "error",
      message: "Model is required.",
      hint: "Select or enter an Anthropic model before testing the environment.",
    });
  }
  ...
  if (canProbe) {
    try {
      ...
      if (!response.ok) {
        ...
        checks.push({
          code: /\bhello\b/i.test(body)
            ? "anthropic_api_probe_passed"
            : "anthropic_api_probe_unexpected_output",
          level: /\bhello\b/i.test(body) ? "info" : "warn",
          message: /\bhello\b/i.test(body)
            ? "Anthropic hello probe succeeded."
            : "Anthropic responded, but the hello probe returned unexpected output.",
      ...
      checks.push({
        ...
        level: "error",
        message: "Anthropic endpoint could not be reached.",
